...
async def register_user(message: Message):
    ...
    user = await db.add_user(referral=referral)
    id = user.id
...
async def buying_item(call: CallbackQuery, callback_data: dict, state: FSMContext):
    ...
    await state.update_data(item=item,
                            purchase=database.Purchase(item_id=item_id,
...
async def enter_quantity(message: Message, state: FSMContext):
    ...
    await message.answer(_('Хорошо, вы хотите купить <i>{quantity}</i> {name} '
                           'по цене <b>{price:,}/шт.</b>\n\n'
                           'Получится <b>{amount:,}</b>. '
                           'Подтверждаете?').format(quantity=quantity,
                                                    name=item.name,
                                                    amount=amount / 100,
                                                    price=item.price / 100
                                                    ),
...
# Отменить во время покупки - убираем все
...
async def approval(call: CallbackQuery, state: FSMContext):
    ...
    await bot.send_message(chat_id=call.from_user.id,
                           text=_('Хорошо. Оплатите <b>{amount:,}</b> по методу указанному ниже '
                                  'и нажмите на кнопку ниже').format(amount=purchase.amount / 100))
    currency = 'UAH'
    need_name = True
